...
function ShellSort() {
        ...
        while (gap > 0) {
            for (let i = gap; i < n; i++) {
                ...
                for (j = i; j >= gap && arr[j - gap] > temp; j -= gap) {
                    ...
                    arr[j] = arr[j - gap];
                    setState(prevState => ({ ...prevState, data: [...arr], movingIndices: [j, j-gap] }));
                    await new Promise(resolve => setTimeout(resolve, delay));
                }
    ...
    return (
        <div className='flex flex-col justify-center items-center h-screen w-full space-y-4 pt-12'>
            <h1 className='text-4xl sm:text-6xl my-16 sm:my-8 sm:mb-16'>Shell Sort</h1>
            ...
            <div className='flex flex-col-reverse sm:flex-row gap-4 w-full max-w-xl py-10'>
                <div className='flex justify-center gap-4 w-full'>
                    <button className='px-4 py-1 text-2xl bg-customLightBlue rounded-lg' onClick={startShellSort}>Sort</button>
