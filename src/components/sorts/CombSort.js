...
function CombSort() {
    ...
    const highlightAllBarsSequentially = () => {
        ...
        for (let i = 0; i < state.data.length; i++) {
            setTimeout(() => {
                setState(prevState => ({ ...prevState, activeIndices: [], completedIndices: [...prevState.completedIndices, i] }));
            }, i * delay);
        }
    };
    ...
    const combSort = async () => {
        ...
        while (!sorted) {
            ...
            for (let i = 0; i + gap < arr.length; i++) {
                ...
                setState(prevState => ({ ...prevState, activeIndices: [i, i + gap] }));
        ...
        <div className='flex flex-col justify-center items-center h-screen w-full space-y-4 pt-12'>
            ...
            <div className='flex flex-col-reverse sm:flex-row gap-4 w-full max-w-xl pt-10'>
                <div className='flex justify-center gap-4 w-full'>
                    <button className='px-4 py-1 text-2xl bg-customLightBlue rounded-lg' onClick={combSort}>Sort</button>
